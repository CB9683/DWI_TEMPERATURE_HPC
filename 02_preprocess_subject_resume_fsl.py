#!/usr/bin/env python3
"""
Resume preprocessing from existing dwi_preproc.mif - FSL version
This script uses FSL for bias correction instead of ANTs
"""
...
def main():
    parser = argparse.ArgumentParser(description="Resume preprocessing with FSL bias correction")
    ...
    parser.add_argument("--skip_bias", action="store_true", help="Skip bias correction if ANTs not available")
    args = parser.parse_args()

    # Setup output directories and logging
    sub_out_dir = os.path.join(args.output_dir, args.subject_id)
    os.makedirs(sub_out_dir, exist_ok=True)
    log_file = os.path.join(sub_out_dir, 'log_02_preprocessing_resume_fsl.txt')
    ...
    if not os.path.exists(preproc_dwi_unbiased):
        if args.skip_bias:
            logger.info("Step 5: SKIPPING bias field correction (--skip_bias flag set)")
            logger.info("Creating symlink to use preprocessed data as is")
            # Create a symlink or copy
            run_command(['cp', preproc_dwi, preproc_dwi_unbiased], log_file, logger)
        else:
            logger.info("Step 5: Bias field correction using FSL")
            # Try FSL-based bias correction
            try:
                run_command(['dwibiascorrect', 'fsl', preproc_dwi, preproc_dwi_unbiased, '-force'], log_file, logger)
            except:
                logger.warning("FSL bias correction failed, trying to continue without bias correction")
                logger.info("Copying preprocessed data without bias correction")
                run_command(['cp', preproc_dwi, preproc_dwi_unbiased], log_file, logger)
    else:
    ...
    summary = {
        ...
        'bias_correction': 'FSL' if not args.skip_bias else 'skipped',
        'processing_steps_resumed': [
            'Bias field correction (FSL)', 'Brain masking', 'Upsampling to 1.5mm',
